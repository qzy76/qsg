$(function(){
	...
	$pintuercheck=function(element,type,value){
		...
		switch(type){
			case "required":return /[^(^\s*)|(\s*$)]/.test($pintu);break;//有输入就行
			case "chinese":return /^[\u0391-\uFFE5]+$/.test($pintu);break;//中文
			case "number":return /^\d+$/.test($pintu);break;//数字
			case "integer":return /^[-\+]?\d+$/.test($pintu);break;//正负数字
			case "fu":return /^[-]?\d+$/.test($pintu);break;//负数字
			case "plusinteger":return /^[+]?\d+$/.test($pintu);break;//正数
			case "double":return /^[-\+]?\d+(\.\d+)?$/.test($pintu);break;//带小数点正负数
			case "plusdouble":return /^[+]?\d+(\.\d+)?$/.test($pintu);break;//带小数点正数
			case "english":return /^[A-Za-z]+$/.test($pintu);break;//字母
			case "username":return /^[a-z]\w{3,}$/i.test($pintu);break;//用户名
			...
			case "email":return /^[^@]+@[^@]+\.[^@]+$/.test($pintu);break;//邮箱
			case "url":return /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/.test($pintu);break;
			case "ip":return /^[\d\.]{7,15}$/.test($pintu);break;//ip地址
			case "qq":return /^[1-9]\d{4,10}$/.test($pintu);break;
			case "currency":return /^\d+(\.\d+)?$/.test($pintu);break;
			case "zip":return /^[1-9]\d{5}$/.test($pintu);break;
			case "zo":return /^[0-1]$/.test($pintu);break;//0或者1   2018-02-02 09:59:59
			case "date":return /^[1-9]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])\s+(20|21|22|23|[0-1]\d):[0-5]\d:[0-5]\d$/;break;//日期
			case "radio":
