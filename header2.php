...
<div class="mtlogin">
		...
		<p style="width: 100%;height: 100px;line-height: 100px;display: block; font-size: 16px;">
		...
		<form action="checkuser.php" method="post">
			...
			<div>
				<p <
					<?php
					if(strpos($file,'product.php') !== false){echo ' style="margin: 10px;margin-top:-10px;"';}?>>
				<input type="text" name="uname" class="user" placeholder="手机 / 邮箱 / 用户名" />
			</p>
			<p <?php if(strpos($file,'product.php') !== false){echo ' style="margin: 10px;"';}?>>
				<input type="password" name="upwd" class="user" placeholder="密码" />
			</p>
			<p <?php if(strpos($file,'product.php') !== false){echo ' style="margin:0 10px;"';}?>>
...
<div id="header" class="clearfix" <?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo 'style="background-color: '.$whorow['pacolor'].';"';} ?>>
			<!--顶部栏-->
			<div class="hnav_title">
				<div class="hnav_center">
					<div class="hnav_l fl">
						...
						<span id="welcome" class="c666"><a class="c666 pr15">欢迎来轻松购</a></span>
						<span id="user" class="ml15">
							<?php
								...
								$file = basename($path);
								if($uid!=null&&$uid>0){
									echo('<a target="_blank" href="user/userCenter.php" class="login co pr15">'.$_COOKIE["ckuname"].'，您好！</a><a href="logout.php?page='.$file.'">退出登录</a>');
								}else{
									echo('<a target="_blank" href="javascript:;" onclick="loginnow ()" class="login co pr15">请登录</a><a target="_blank" href="register.php" class="reg c666">免费注册</a>');
								}
							?>
						</span>
						<input type="hidden" id="islogin comeBack" value="<?php echo $uid;?>"/>
					</div>
					<div class="hnav_r">
						<ul class="person_new fl tn">
							...
							<li><a class="aba" target="_blank" <?php if($uid!=null&&$uid>0){echo('href="user/userCenter.php"');}else{echo('onclick="loginnow ()" data-comeBack="user/userCenter.php"');}?>>个人中心</a></li>
			...
			<div class="hmain_center clearfix h200 pr <?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo '"style="background-color: '.$whorow['pacolor'].';"';}else{echo 'bafff"';} ?>>
				<div class="logo bac pt15">
					<?php 
						$path = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
						$file = basename($path);
						if($file!="index.php"||$file!="product.php"){echo '<a target="_blank" href="index.php" >';}else{echo '<a href="javascript:;">';}
					?><img src="<?php
					if(strpos($file,'pages.php') !== false){
						echo "img/PNGlogo.png";
					}else{
					 echo('uploads/'.$prorow['ilogo']);}?>" alt="网页logo" title="轻松购-http://qzy.is-great.net" height="90"/></a>
				</div>
				<!--搜索框-->
				<div class="search pa">
					<form action="goodslist.php" method="get">
						<div class="search_form">
							<div class="search_title fl">
								<input type="text" name="skey" class="skey search_pro w390" 
									<?php 
										$path = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
										$file = basename($path);
										if($file=="cart.php"||$file=="order.php"||$file=="orderlist.php"){echo 'style="height: 24px;"';}
										if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false||strpos($_SERVER["REQUEST_URI"],'goodslist.php') !== false){echo 'style="height: 24px;"';}
									?>
									 placeholder="<?php echo($prorow['iseacsh']);?>" />
							</div>
					...
					<div class="search_push" <?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo 'style="color:#000;"';} ?>>
						<ul class="push_pro fl tn">
							<li class="fl fb pr15<?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo ' c333"';}else{echo ' c666"';} ?>>热品推荐:</li>
							<?php
								require_once("connect.php");
									$rsql="SELECT ccconame FROM product INNER JOIN ccclassification ON 
									pclassification = cccoid WHERE phot =1 order by rand() LIMIT 0,11";
									$rresult=mysql_query($rsql);
									while($rrow=mysql_fetch_assoc($rresult)){ ?>
										<li><a <?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo 'class="c333"';}?> target="_blank" href="goodslist.php?class3=<?php echo($rrow['ccconame']);?>"><?php echo($rrow['ccconame']);?></a></li>
									<?php }?>
						</ul>
					</div>
				</div>
...
if($file=="index.php"){?>
				...
				<?php } ?>
				<div class="main-hnav <?php if($file=="index.php"){
					echo 'pa"';
				}?> mt10 bafff "<?php 
				if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo 'style="background-color: '.$whorow['pacolor'].';"';}
				?>>
					<ul <?php
					if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo 'style="background-color: '.$whorow['pacolor'].';width: auto;"';}
							if($file!="index.php"){
								echo'style="width: 1200px;overflow-x: inherit;"><li class="fl lhh40 h40 f16">';
							}else{
								echo'<li class="fl lhh40 h40 f16 pc20">';
							};
							
							?>
						<a href="index.php" class="fb tdn<?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo ' c333"';}else{echo ' c666"';}?>>首页</a></li>
						<?php
							if($file=="index.php"){
								$len = "5";
							}else{
							...
							$nsql="select * from classification order by cosort limit 0,$len";
							$nresult=mysql_query($nsql);
							while($nrow=mysql_fetch_assoc($nresult)){
							?>
						<li class="fl lhh40 h40 f16 pc20"><a class="fb tdn<?php if(strpos($_SERVER["REQUEST_URI"],'pages.php') !== false){echo ' c333"';}else{echo ' c666"';}?> target="_blank" href="goodslist.php?class1=<?php echo($nrow['coname']);?>"><?php echo($nrow['coname']);?></a></li>
						<?php }?>
					</ul>
				</div>
				<?php 
